...
if( networkType == imageNet::ALEXNET )
		return init( "networks/alexnet.prototxt", "networks/snapshot_iter_1170.caffemodel", NULL, "networks/labels.txt", IMAGENET_DEFAULT_INPUT, IMAGENET_DEFAULT_OUTPUT, maxBatchSize );
	else
		return init( "networks/googlenet.prototxt", "networks/snapshot_iter_1170.caffemodel", NULL, "networks/labels.txt", IMAGENET_DEFAULT_INPUT, IMAGENET_DEFAULT_OUTPUT, maxBatchSize );
}
...
bool imageNet::loadClassInfo( const char* filename )
{
	...
	{
		const int syn = 0;  // length of synset prefix (in characters)
